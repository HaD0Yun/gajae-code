...
| External controller / bot | SDK WebSocket for a live session; Coordinator service (`gjc mcp-serve coordinator`) for multi-session orchestration; `gjc daemon session` CLI for scripts | External controllers drive GJC through the SDK's loopback WebSocket protocol (`docs/sdk.md`) or the Coordinator service, not scrollback scraping. |
