...
- `--write --stage <type> --stage_n <N> --artifact <markdown file path or markdown string> [--execution-plan <json-or-path>]`: Native writer for Planner/Architect/Critic/revision/ADR/final pending-approval markdown under `.gjc/_session-{sessionid}/plans/ralplan/<run-id>/`; the optional execution plan is valid only for `final` and writes `.gjc/_session-{sessionid}/plans/ralplan/<run-id>/execution-plan.json`. Do not edit `.gjc/` directly.
...
# final stage with an Ultragoal execution sidecar:
gjc ralplan --write --stage final --stage_n <N> --artifact-env GJC_RALPLAN_ARTIFACT \
  --execution-plan '<json-or-path>' --json
```

Use stages `planner`, `architect`, `critic`, `revision`, `post-interview`, `adr`, or `final`; increment `--stage_n` each consensus pass. The writer accepts inline markdown, an artifact path prepared outside `.gjc/`, or `--artifact-env GJC_RALPLAN_ARTIFACT`, persists `stage-<NN>-<stage>.md` plus `index.jsonl` under `.gjc/_session-{sessionid}/plans/ralplan/<run-id>/`, and copies `final` to `pending-approval.md`. On the final write only, `--execution-plan <json-or-path>` accepts a version 1 execution-plan JSON object and persists its normalized form beside `pending-approval.md` at `.gjc/_session-{sessionid}/plans/ralplan/<run-id>/execution-plan.json`; the JSON receipt includes `execution_plan_path`. Final publication is serialized per run and content-addressed by the final artifact SHA-256: all requested members are checked before publication, the index row is appended last, identical retries are no-ops, missing members are safely repaired, and any conflicting member fails closed. Sidecar lane IDs and every `after` reference use the same non-empty alphanumeric grammar, and duplicate lane IDs or duplicate dependency references are invalid. Ralplan mutation blocking is enforced in code; use temp directories (`os.tmpdir()`/`$TMPDIR`, `/tmp`, `/var/tmp`) only for oversized scratch artifacts, never the repo or `.gjc/`.
...
1. **Planner** creates the initial plan and a compact **RALPLAN-DR summary** before review. Launch the Planner ONCE per run as a detached, resumable subagent (await it before the Architect) and record its returned subagent id as the run's persisted Planner id; persist the stage with `gjc ralplan --write --stage planner --stage_n 1 --artifact-env GJC_RALPLAN_ARTIFACT --planner-id <id> --planner-resumable <true|false>` (see **Persisted Planner** below):
   ...
   - **Execution staffing classification**: classify every implementation step as `parallel` or `sequential`, state the dependency that forces each sequential edge, and reject fake parallelism using the Big Five checks: shared files, shared mutable state, producer/consumer data, ordering-sensitive APIs, and overlapping verification surfaces.
   - Every execution plan MUST include an independent verifier lane. The verifier may depend on implementation lanes but MUST not be combined with the implementation it verifies.
   - Use exactly this lane-heading grammar: `### Lane <label> — <title>` with the optional dependency suffix after the title, as in `### Lane B — Verification (after: A)`. Labels are alphanumeric and dependencies are comma-separated labels (`after: A, C`); every referenced label must exist, and self-dependencies or cycles are invalid.
...
3. **Review fan-out after Planner persistence**: launch fresh Architect and Critic review lanes against the same immutable Planner receipt/path/sha/stage_n when Critic is **plan-only** and does not consume Architect output.
   - **Architect lane**: challenge architecture, surface tradeoff tensions, and enrich thin plans with synthesis or missed sub-scope. Explicitly inspect shared files, shared mutable state, public APIs, schemas, generated artifacts, and verification surfaces across lanes; any overlap must become a dependency or a single owner, not optimistic parallel work. Persist with `gjc ralplan --write --stage architect --stage_n <N> --artifact-env GJC_RALPLAN_ARTIFACT --json`, then return receipt/path plus `CLEAR`/`WATCH`/`BLOCK` and `APPROVE`/`COMMENT`/`REQUEST CHANGES`.
   - **Plan-only Critic lane**: independently check quality, principle-option consistency, alternatives, risks, acceptance criteria, and verification. Re-run all Big Five checks (shared files, shared mutable state, producer/consumer data, ordering-sensitive APIs, and overlapping verification surfaces), reject fake parallelism, and require explicit dependencies or ownership corrections. When the plan is thin, request concrete expansion rather than only defects. Persist with `gjc ralplan --write --stage critic --stage_n <N> --artifact-env GJC_RALPLAN_ARTIFACT --json`, then return receipt/path plus `OKAY`/`ITERATE`/`REJECT`.
...
8. **Always** present the finalized plan via the `ask` tool (regardless of `--interactive`) with `workflowGate: { stage: "ralplan", kind: "approval" }` on the final question so RPC/headless clients receive a `ralplan`/`approval` workflow gate, not a deep-interview question gate. Use these options:
   ...
   - **Approve execution via team** — independent coordinated workers; tmux is the default backend and explicit standalone headless Team is supported
...
9. On approval: invoke `/skill:ultragoal` for execution by default; invoke `/skill:team` when the user explicitly selects independent coordinated Team execution. Team defaults to tmux, while standalone Team may explicitly use `GJC_TEAM_BACKEND=headless`. Automatic headless Team launch is only the Ultragoal `GJC_ULTRAGOAL_PARALLEL` opt-in path. On **Refine further**, return to the step 5 re-review loop. On **Stop here**, leave the `pending approval` artifact and stop. Never implement directly.
...
- **team**: independent coordinated workers; tmux is the default backend, and explicit standalone headless Team is supported
